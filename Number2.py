...
gc = gspread.service_account(filename='active-valve-365018-0f39d3352474.json')
sh = gc.open("LinearRegressionSheets")
...
for i in range(len(x)):
    ...
    new_y = float(model(a, b, x[i]))
    sh.sheet1.update(('D' + str(i + 1)), str(new_y).replace('.', ','))
    sh.sheet1.update(('E' + str(i + 1)), str(abs(new_y - y[i])).replace('.', ','))
    print(abs(new_y - y[i]))
# print(a, b, loss)
# plt.scatter(x, y)
